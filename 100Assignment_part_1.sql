...
--Qn 1. Write a query to select all data from the `Customers` table.
...
14.-- 14. Write a query to join the `Sales` and `Products` tables on product_id, and Show product name and total sales amount per product
...
SELECT p.product_name, p.price, s.sale_id
FROM assignment.sales s
JOIN assignment.products p ON s.product_id = p.product_id
...
47-- 47. Write a query to find the products that have not been sold in the last 6 months.
